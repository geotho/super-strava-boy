...
const cross =
  "https://storage.googleapis.com/material-icons/external-assets/v4/icons/svg/ic_close_black_16px.svg";
...
function initMap() {
  ...
  fetchCommutes("to_work")
    ...
    .then((activities) => {
      const minTime = activities
        .filter(Boolean)
        .map((a) => a.stream[0].time)
        .sort()[0];
      ...
      console.log(
        streams.filter(Boolean).map((s) => posAtTimeT(s, 1447666275))
      );
    });
}
...
function fetchCommutes(direction) {
  const p = fetch("activities200.json").then((x) => x.json());

  if (direction === "to_work") {
    // get the morning commutes
    return p.then((acts) =>
      acts.filter((a) => new Date(a.start_date).getHours() < 12)
    );
...
function fetchActivityLocal(id) {
  return fetch("scripts/pulldata/out/" + id + ".json").then((x) => x.json());
}
...
function greatestIndexLessThanT_v1(activity, t) {
  ...
  while (
    i < activity.length - 1 &&
    !(activity[i].time <= t && t <= activity[i + 1].time)
  ) {
    i++;
  }
